...
$(document).ready(function(){
	...
	$('.workout-btns').click(function(){
		...
		if(this.innerHTML == 'Multiple Movements' || this.innerHTML == 'Single Movement')
		{
			if(this.innerHTML == 'Multiple Movements')
			{
				template = 'multiple';
				$(".selected-exercise-type").remove();
			}
			else
				template = 'single';
			...
			$.ajax({                    
			    url:'/templates/getMovementTemplates',
			    type:"POST",                                        
			    data:_params,
			    success: function(data) {
	      			$('.templates-btns').html(data);
			    }
			});


			var update_params = {
				exercise_type : this.innerHTML,
				template: template
			}

			$.ajax({                    
			    url : '/exercises/save_custom_workout',
			    type : "POST",                                        
			    data : update_params,
			    success: function(data) {
	...
	$('.exercise-type-selected').click(function(){
		...
		name = '<div class="attirbute-name-label">'+this.innerHTML+'</div>';
		...
		$.ajax({
	        ...
	        success: function(data) {
	          	$('.selected-exercise-type-content').html('<div class="main-exercise">'+name+inputField+data+'</div>');
	        }, // callback function
	        error: function() {
...
function saveActivity(element)
{
	var attr = {};
	count = 0;

	html = '<div class="main-exercise-blocks">';
		html += '<div class="saved-exercise-list-name">'+$('.attirbute-name-label').html()+'</div>';

		attr = {
			name : $('.attirbute-name-label').html(),
			attribute : {
		...
		$('.AttributeContainer').each(function () {
			...
			if($('.AttributeInput', this).children('input').val() != 0)
			{
				attrName = $('.AttributeLabel', this).html();
				updateAttrName = attrName.replace(' :', '');
				attr['attribute'][Object.keys(attr['attribute']).length] = {
					name : updateAttrName,
					value : $('.AttributeInput', this).children('input').val()
				}

				html += '<div class="saved-exercise-list-attribute">';
					html += '<div class="saved-exercise-list-attribute-name">'+$('.AttributeLabel', this).html()+'</div>';
					html += '<div class="saved-exercise-list-attribute-value">'+$('.AttributeInput', this).children('input').val()+'</div>';
					html += '<div class="clear"></div>';
				html += '</div>';
			}
	...
	$('.main-exercise').remove();
	$('.saved-exercise-list').append(html);

	$.ajax({
        url: "/exercises/update_attributes", // url to send data to
        type: "POST", // parse data as type
        data: attr, // data object to parse
        dataType: "json", // return type
        success: function(data) {
...
function saveWorkout()
{
	...
	$('.template-details').each(function () {
		...
		if($('.attribute-input', this).children('input').val() != 0 || $('.attribute-input', this).children('input').val() != '' || $('.attribute-input', this).children('input').is(':checked'))
		{
			templateDetailsName = $('.attribute-label', this).html();
			data['template_details'][Object.keys(data['template_details']).length] = {
				name : templateDetailsName,
				value : $('.attribute-input', this).children('input', this).val()
			}
		}
	...
	$.ajax({
        url: "/exercises/save_workout_complete", // url to send data to
        type: "POST", // parse data as type
        data: data, // data object to parse
        dataType: "json", // return type
        success: function(data) {
        	if(data == true)
    			document.location = '/members/workouts/personal';
    		else
